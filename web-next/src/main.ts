...
import { type LayoutGraph, type LayoutPoint } from './layout/layoutTypes';
...
import {
  crossingDiagnosticsForGraph,
  drawLasso as drawCanvasLasso,
  drawLayoutGraphOn as drawCanvasLayoutGraphOn,
  drawPendingGraph,
  drawPlainEquals,
  drawQuestionEquals,
  inPanel,
  ...
  roundedRectPath,
  screenNodeRect,
  viewForLayout as viewForLayoutOnCanvas
} from './app/diagramCanvas';
...
const viewForLayout = (g: LayoutGraph, panel: Rect, zoomFactor = zoom): View => viewForLayoutOnCanvas(g, panel, zoomFactor);

const drawLayoutGraphOn = (c: CanvasRenderingContext2D, graph: LayoutGraph, panel: Rect, selected: Set<string>, view = viewForLayout(graph, panel)) => {
  drawCanvasLayoutGraphOn({ ctx: c, graph, panel, selected, view, cssVar, showNodeLabels: SHOW_NODE_LABELS });
  return view;
};

const drawLayoutGraph = (graph: LayoutGraph, panel: Rect, selected: Set<string>, view?: View) => drawLayoutGraphOn(ctx, graph, panel, selected, view);
...
const render = (refresh = true) => {
  ...
  else drawPendingGraph(ctx, panels.lhs);
  if (rhs && rhsView) drawLayoutGraph(rhs, panels.rhs, selectedRhs, rhsView);
  else drawPendingGraph(ctx, panels.rhs);
  ...
  drawCanvasLasso({ ctx, lasso, dragging });
  endCanvas();
  if (refresh) {
